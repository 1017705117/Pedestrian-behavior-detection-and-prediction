...
def vis_detections(im, class_name, ax, dets, thresh=0.5):
    ...
    for i in inds:
        bbox = dets[i, :4]
        score = dets[i, -2]
        class_ind = int(dets[i, -1])
        print(class_ind)
        name = class_name[class_ind]
...
def demo(sess, net, image_name):
    ...
    for cls_ind, cls in enumerate(CLASSES[1:]):
        ...
        dets = np.hstack((dets,
                          cls_int[:,np.newaxis])).astype(np.float32)
        if ht.shape[0]>0:
            ht = np.vstack((ht,
...
if __name__ == '__main__':
    ...
    im_names = ['000369.jpg', '002345.jpg', '002818.jpg', '003950.jpg']
